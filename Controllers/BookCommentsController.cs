...
namespace Bakalauras.Controllers
{
    ...
    public class BookCommentsController : ControllerBase
    {
        ...
        public BookCommentsController(ICommentRepository repo, IAuthorizationService authService,
            UserManager<BookieUser> userManager)
        {
        ...
        public async Task<IEnumerable<CommentDto>> GetMany(int bookId)
        {
            ...
            return comments.Select(x => new CommentDto(x.Id, x.BookId,_Type, DateTime.Now, x.Content, x.UserId, x.Username))
                           .Where(y => y.EntityId == bookId);
        }
        ...
        public async Task<ActionResult<CommentDto>> Get(int bookId, int commentId)
        {
            ...
            return new CommentDto(comment.Id, comment.BookId,_Type, DateTime.Now, comment.Content,
        ...
        public async Task<ActionResult<CommentDto>> Create(CreateCommentDto createCommentDto, int bookId)
        {
            ...
            var comment = new Comment { Type=_Type,Content = createCommentDto.Content, Date = DateTime.Now, Username = user,
                UserId = User.FindFirstValue(JwtRegisteredClaimNames.Sub) };

            await _CommentRepository.CreateAsync(comment, bookId,_Type);

            //201
            return Created("201", new CommentDto(comment.Id, comment.BookId,_Type, comment.Date, comment.Content,
                comment.UserId, comment.Username));
        }
        ...
        public async Task<ActionResult<CommentDto>> Update(int commentId, int bookId, UpdateCommentDto updateCommentDto)
        {
            ...
            return Ok(new CommentDto(comment.Id, comment.BookId,_Type, DateTime.Now, comment.Content, comment.UserId, comment.Username));
        }
